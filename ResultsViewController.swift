...
class ResultsViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        CKContainer(identifier: "iCloud.com.yourcompany.Project33-CloudKit").publicCloudDatabase.perform(query, inZoneWith: nil) { [unowned self] (results, error) in
            if let error = error {
    ...
    func add(suggestion: String) {
        ...
        CKContainer(identifier: "iCloud.com.yourcompany.Project33-CloudKit").publicCloudDatabase.save(whistleRecord) { [unowned self] (record, error) in
    ...
    @objc func downloadTapped() {
        ...
        CKContainer(identifier: "iCloud.com.yourcompany.Project33-CloudKit").publicCloudDatabase.fetch(withRecordID: whistle.recordID) { [unowned self] (record, error) in
            if let error = error {
                DispatchQueue.main.async {
                    let ac = UIAlertController(title: "Error", message: "There was a problem fetching the whistle: \(error.localizedDescription)", preferredStyle: .alert)
                    ac.addAction(UIAlertAction(title: "OK", style: .default))
                    self.present(ac, animated: true)
